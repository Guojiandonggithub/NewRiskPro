...
public class SocketService extends Service {
    ...
    private Timer recvtimer = new Timer();
                ...
                public void run() {
                    ...
                    try {
                        ...
                        if (e instanceof SocketTimeoutException) {
                            //toastMsg("连接超时，正在重连");
                            Data.getInstance().setConnect(false);
                            releaseSocket();
        ...
        if (task == null) {
            task = new TimerTask() {
                ...
                public void run() {
                    try {
                        outputStream = socket.getOutputStream();
                        //toastMsg("链接成功，正在发送数据");
                        /*这里的编码方式根据你的需求去改*/
                        Data.getInstance().setConnect(true);
    ...
    /*定时接收取数据*/
    private void recvBeatData() {
        if (recvtimer == null) {
            recvtimer = new Timer();
        }

        if (recvtask == null) {
            recvtask = new TimerTask() {
                @Override
                public void run() {
                    try {
        ...
        recvtimer.schedule(recvtask, 0, 5000);
    }
